...
class TeacherAccountCreattionTest(TestCase):
    ...
    def setUp(self):
        self.user = get_user_model().objects.create_user(username='test1',is_teacher=True,
                                                         password='12test12',
                                                         email='test@example1.com', first_name="test", last_name="test")
        self.user.save()
    ...
    def testWrongPassword(self):
        user = authenticate(username='test1', password='wrong')
        self.assertFalse(user is not None and user.is_authenticated)
